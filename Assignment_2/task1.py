	...
	def one_and_bucket(self, input_params):
		...
		for basket_items in baskets_list:
			...
			for pair_items in combinations(basket_items, 2):
				hash_value = self.hash_pair(int(pair_items[0]), int(pair_items[1]))
				bucket[hash_value]+=1
				...
				if(len(baskets_list[i])>=tuple_number):
					if tuple_number == 2:
						for item_tuple in combinations(baskets_list[i], 2):
							hash_value = self.hash_pair(int(item_tuple[0]), int(item_tuple[1]))
...
class Building_Structure:
	...
	def read_input(self):
		global case_number, support, input_file, output_file
	...
	def create_RDD(self, rdd, case_number):
		...
		if case_number == '1':
			...
			rdd_lines_users = rdd.map(lambda line: (line.split(',')[1], line.split(',')[0]))
			rdd_users_group = rdd_lines_users.groupByKey()
			rdd_users = rdd_users_group.map(lambda line: (line[0], self.modify(list(line[1]))))
			#print(rdd_users.collect())
			return rdd_users
...
def main():
	...
	rdd_process = building_structure.create_RDD(rdd_lines_reqd, case_number) 
