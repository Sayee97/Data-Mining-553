...
class Building_Model:
	...
	def make_rdd(self, rdd_lines):
		...
		rdd_total_count = rdd_count_rdd.values().sum()


		rdd_count_words_filter = rdd_count_rdd.filter(lambda a: a[1]<0.000001*rdd_total_count)

		# for extracting words!!!!!!!!
		rdd_words_entire_doc = rdd_pair_filter.flatMap(lambda x:x)
		print(rdd_words_entire_doc.take(1))
